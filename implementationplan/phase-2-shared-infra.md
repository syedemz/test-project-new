...
stories:
  - id: 2.0
    ...
    acceptance_criteria:
      ...
      - "`tsconfig.json` configured with a `paths` alias mapping `@/*` → `src/*` and `baseUrl` set so `theme.md`'s import pattern (`import { useTheme } from '@/theme'`) resolves correctly. Verified with `npx tsc --noEmit`."
      - "`jest.config.js` configured with a matching `moduleNameMapper` so tests can `import` from `@/*` and resolve to `src/*`."
    ...
    notes: "This is a prep story — no application code is written here. Its job is to leave the repo in a state where every subsequent phase 2 story can place its file under `src/` without colliding with phase 1's placeholder scaffold."
